...
class RegularizerModel(DenoisingModel):
    ...
    def _add_keys_to_config(self):
        """
        Keys added to config if self.add_keys is True, gives example of config for approximating curvature
        with TNRD regularizer.
        """
    ...
    def _train(self):
        ...
        # TV does not need to be trained
        if not self.config['R']['name'] == 'TV':
