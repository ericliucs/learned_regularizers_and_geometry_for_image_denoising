...
from denoising.generator.datagenerator import DataGenerator
import numpy as np
from denoising.training.losses import retrieve_loss_function
from tensorflow.keras import Input, Model
from denoising.util import get_num_channels, psnr, rmse
from tensorflow.keras.callbacks import CSVLogger, ModelCheckpoint, LearningRateScheduler
from denoising.training.callbacks import PSNRTest
...
class DenoisingModel(ABC):
    ...
    def _standard_training(self, save_every: int, test_every: int):
        """Standard training for vnet denoising model. Adds functionality to update non-trainable weights for
    ...
    def test(self):
        ...
        # Get data range
        data_range = 1.0
        if self.config['train_task'] == 'denoise_curvature' or 'curvature' in self.config['loss_function']:
        ...
        if 'curvature' in self.config['loss_function']:
            func = TVRegularizer({}).grad
        else:
            def func(item):
                return item
